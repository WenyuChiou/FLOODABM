# -*- coding: utf-8 -*-
# modules/actions/pipeline.py (fast)
...
from .tp import (TPConfig, TPGroupParams, update_tract_tp_owner_renter, update_household_tp)  # type: ignore
...
def run_one_year_mgmix_fast(
    *,
    ...
    draw_bounds: dict[str, tuple[float, float]] | None = None,
    # Legacy params (ignored — kept for call-site compatibility during transition)
    tract_psy: pd.DataFrame | None = None,
    idxer=None,
    w_vec: np.ndarray | None = None,
):
    """
    Return (decisions_df, state_next, changes) using household-level psychology.

    Psychology is now stored per-household in STATE columns (TP, CP_init, SP_init, etc.).
    No tract-level TRACT_PSY, no w_vec mixing.
    """
    # 0) Update household-level TP (decay + shock)
    state = update_household_tp(
        state=state,
    ...
    # 1) Predict per-group (no mixing)
    probs = predict_by_group(state, predictor_owner, predictor_renter)

    # 2) Hierarchical decision (owner: FI->EH->BP; renter: FI->RL)
    ...
    # 3) Assemble output decision table
    base = state[["i","group","tract_geoid"]].reset_index(drop=True)
    dec  = base.join(dec)
    dec.insert(3, "year", year)

    # 3.5) If depth is provided, select shallower destination for RL
    if depth_m_by_tract:
...
def run_loop_mgmix_fast(
    ...
    # Legacy (ignored)
    tract_psy: pd.DataFrame | None = None,
    owner_share_by_tract: Dict[str, float] | None = None,
) -> Dict[int, pd.DataFrame]:
    """Multi-year loop using household-level psychology."""
    rng = np.random.RandomState(seed)
    decisions_by_year: Dict[int, pd.DataFrame] = {}

    for y in years:
        ratio_y = ratio_provider(y) if ratio_provider else {}
        dec, state, _ = run_one_year_mgmix_fast(
            year=y,
            state=state,
