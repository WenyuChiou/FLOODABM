...
Creates plots showing TP evolution over time for MG and NMG groups.
"""
...
def plot_tp_outputs(tp_traj: pd.DataFrame, fig_root: Path) -> None:
    ...
    Args:
        tp_traj: DataFrame with columns: year, tract_geoid, TP_MG, TP_NMG, phase
    ...
    _plot_median_iqr(tp_traj, "TP_MG", "Trust in Policymaker (MG)", tp_dir / "tp_mg_median_iqr.png")
    _plot_median_iqr(tp_traj, "TP_NMG", "Trust in Policymaker (NMG)", tp_dir / "tp_nmg_median_iqr.png")
    
    # Plot area change
    _plot_area_change(tp_traj, "TP_MG", "TP Change Distribution (MG)", tp_dir / "tp_mg_area_change.png")
    _plot_area_change(tp_traj, "TP_NMG", "TP Change Distribution (NMG)", tp_dir / "tp_nmg_area_change.png")
...
def _plot_median_iqr(
    ...
    Args:
        ...
        col: Column to plot (TP_MG or TP_NMG)
        title: Plot title
...
def _plot_combined_tp(tp_traj: pd.DataFrame, out_path: Path) -> None:
    """Plot combined MG and NMG TP trajectories.
    
    Args:
        tp_traj: TP trajectory data
        out_path: Output file path
    """
    if "TP_MG" not in tp_traj.columns or "TP_NMG" not in tp_traj.columns:
        return
    
    fig, axes = plt.subplots(1, 2, figsize=(14, 5), sharey=True)
    
    for ax, (col, group_name, color) in zip(axes, [
        ("TP_MG", "Minority Group (MG)", COLORS["blue"]),
        ("TP_NMG", "Homeowners (NMG)", COLORS["green"]),
    ]):
        yearly = tp_traj.groupby("year")[col].agg(["median", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)])
        yearly.columns = ["median", "q25", "q75"]
        years = yearly.index.values
