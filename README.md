...
Use the unified pipeline to regenerate the model figures from existing simulation outputs:

```bash
python generate_paper_figures.py          # run all registered figure scripts
python generate_paper_figures.py --figs 4 6 7  # specific registry entries only
python generate_paper_figures.py --list   # show the figure registry
```

The figure scripts live in `scripts/paper_figures/`. Each plots one part of the analysis. Final figure numbers are assigned in the manuscript, so the table below describes what each script plots rather than a fixed figure number:

| Script | Plots |
|--------|-------|
| `plot_nfip_validation_paper.py` | Simulated vs observed NFIP payout per claim (z-score), county level |
| `plot_fig_rq1_combined.py` | Cumulative ground-up and actual loss, with the financial-outcome panels |
| `plot_fig_cumulative_behavior.py` | Cumulative adaptation-action composition |
| `plot_fig7_v2_2panel.py` | Annual adaptation adoption trajectories (homeowner vs renter) |
| `plot_sa_rt_delta.py` | Sensitivity analysis over the ratio-threshold grid |
| `plot_fig10_tp_boxplot.py` | Threat-perception distribution by year |
| `plot_si_tp_trajectory.py` | Supporting-information threat-perception trajectories |

The study-area map, the Bayesian-procedure diagram, the framework flowchart, the flood-model diagnostic panels, and the spatial flood-insurance maps are prepared outside this pipeline.

> Note: the registry inside `generate_paper_figures.py` still uses the earlier figure numbering and "RQ" labels. The pipeline runs correctly, but those internal indices do not match the current manuscript figure numbers; renumbering the registry is a pending cleanup.
