...
def sequential_decision_fast(
    ...
    Each step: if U(0,1) < prob, take action; otherwise proceed to next option.
    ...
    # Single RNG call for all random draws (5 columns for max actions)
    u = rng.random((N, 5))
    ...
    take_BP = (u[:, 2] < pBP) & rem_owner2
    out_action[take_BP] = "BP"
    
    # ==================== Renter Decision Tree ====================
    # Step 1: FI decision
    take_FI_r = (u[:, 3] < pFI) & is_renter
